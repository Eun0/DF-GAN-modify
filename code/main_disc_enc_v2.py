...
from model import NetG,DISC_ENCODER_V2
...
def train(dataloader,netG,netD,text_encoder,optimizerG,optimizerD,state_epoch,batch_size,device,output_dir,logger):
    ...
    for epoch in range(state_epoch+1, cfg.TRAIN.MAX_EPOCH+1):
        ...
        for step, data in enumerate(dataloader, 0):
            ...
            _,real_features = netD(imgs)
            output = torch.matmul(real_features,sent_emb.transpose(0,1))
            errD_real = torch.nn.ReLU()(1.0 - output).mean()

            output = torch.matmul(real_features[:(batch_size - 1)], sent_emb[1:batch_size].transpose(0,1))
            ...
            _,fake_features = netD(fake.detach()) 
            errD_fake = torch.matmul(fake_features,sent_emb.transpose(0,1))
            ...
            _,features = netD(interpolated)
            out = torch.matmul(features,sent_inter.transpose(0,1))
            ...
            # update G
                ...
                #i = -1
            _,features = netD(fake)
            output = torch.matmul(features,sent_emb.transpose(0,1))
            errG = - output.mean()
            optimizerG.zero_grad()
            optimizerD.zero_grad()
            errG.backward()
...
if __name__ == "__main__":
    ...
    netD = DISC_ENCODER_V2(cfg.TRAIN.NF).cuda()
