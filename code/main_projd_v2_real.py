...
def train(dataloader,netG,netD,text_encoder,optimizerG,optimizerD_enc,optimizerD_proj,state_epoch,batch_size,device,output_dir,logger):
    #i = -1
    for epoch in range(state_epoch+1, cfg.TRAIN.MAX_EPOCH+1):
        ...
        for step, data in enumerate(dataloader, 0):
            ...
            logger.info('[%d/%d][%d/%d] Loss_encD: %.3f Loss_projD: %.3f Loss_G %.3f errD_real %.3f errD_mis %.3f errD_fake %.3f magp %.3f'
                % (epoch, cfg.TRAIN.MAX_EPOCH, step, len(dataloader), errD_enc.item(), errD_proj.item(), errG.item(),errD_real.item(),errD_mismatch.item(),errD_fake.item(),d_loss_gp.item()))
...
if __name__ == "__main__":
    ...
    optimizerD_enc = torch.optim.Adam(netD.feature_encoder.parameters(), lr=0.0004, betas=(0.0, 0.9))
    optimizerD_proj = torch.optim.Adam(netD.COND_DNET.parameters(),lr=0.004,betas=(0.0,0.9))
    ...
    if cfg.B_VALIDATION:
        ...
        train(dataloader,netG,netD,text_encoder,optimizerG,optimizerD_enc,optimizerD_proj, state_epoch,batch_size,device,output_dir,logger)
